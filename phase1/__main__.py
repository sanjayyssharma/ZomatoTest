...
from phase1.recommender import recommend


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="phase1", add_help=True)
    sub = p.add_subparsers(dest="command", required=True)

    rec = sub.add_parser("recommend", help="Deterministic filtering + baseline ranking")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "recommend":
        ...
        console.print("[bold]Phase 1 recommendations (baseline)[/bold]")
        console.print(f"[dim]Candidates:[/dim] {resp.candidate_count} / {resp.total_restaurants}")
        ...
        for i, r in enumerate(resp.top, start=1):
            rating = "—" if r.rating is None else f"{r.rating:.1f}"
            cost = "—" if r.cost_for_two is None else f"{int(r.cost_for_two)}"
            cuisines = ", ".join(r.cuisines[:4]) if r.cuisines else "—"
            loc = r.location or "—"
            console.print(f"{i}. [bold]{r.name}[/bold]  [dim]({loc})[/dim]")
            console.print(f"   rating: {rating} | cost_for_two: {cost} | cuisines: {cuisines}")
